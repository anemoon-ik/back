    ...
    if request.method == "POST":
        ...
        if email != "":
            try:
                ...
                new_password = "".join([random.choice(list("ROBERTPATTINSON13051986")) for i in range(8)])
                user.set_password(new_password)
                user.save()
                text = f"Catch your new password: {new_password}"
                send_email.delay(
                    email=email, 
                    text=text, 
                    subject="Password Recovery:D"
                )
                return redirect("/forgot_password_success")
            except ObjectDoesNotExist:
                print("We suppose some amateur tried this email", email)
                return redirect("/forgot_password_success")
    return render(request, "forgot.html")

def forgot_password_success(request: HttpRequest) -> HttpRequest:
